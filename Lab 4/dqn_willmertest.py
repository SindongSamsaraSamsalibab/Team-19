...
class QNetwork(nn.Module):
    def __init__(self, env):
        ...
        self.network = nn.Sequential(
            nn.Conv2d(kernel_size=8,stride=4),
            nn.ReLU(),
            nn.Conv2d(kernel_size=4,stride=2),
            nn.ReLU(),
            nn.Linear(512,512),
            nn.ReLU(),
        ...
        if global_step > params.learning_starts:
            if global_step % params.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    target_max, _ = max(q_network(obs))
                    td_target = data.rewards + params.gamma*target_max

                old_val = q_network(data.observation).gather(1, data.actions).squeeze()
